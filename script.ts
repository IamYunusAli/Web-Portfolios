...
const blobGroups = document.querySelectorAll("g.blob")
const sectionTags = document.querySelectorAll("section")
const arrowTag = document.querySelector("img.arrow")! as HTMLElement;
...
const fadeHeader = () => {
  ...
  headerTag.style.opacity = (1 - easing(pixels / 500)).toString()
}

const fadeArrow = () => {
  const pixels = window.pageYOffset

  arrowTag.style.opacity = (1 - easing(pixels / 250)).toString()
}
